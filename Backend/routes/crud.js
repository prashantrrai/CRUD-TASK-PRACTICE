...
crudRoute.post("/crudadd", async (req, res) => {
    ...
    //     }
    try {
        const Mydata = new crudModel(req.body);
        await Mydata.save();
        console.log(Mydata);
        return res
            .status(200)
            .json({ success: true, message: "User Added Successfully", Mydata });
    } catch (error) {
        console.log(error);
        if (error.keyPattern) {
            return res.json({
...
// ---------------------GET USER API-----------------------//
crudRoute.get("/onlycruddata", async (req, res) => {
    try {
        const data = await crudModel.find();
        return res
            .status(200)
            .json({ success: true, message: "User Data Got Sucecssfully", data });
    } catch (error) {
        console.log(error);
        return res
            .status(500)
